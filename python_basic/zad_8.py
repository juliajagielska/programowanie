...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pobieranie listy browarów z OpenBreweryDB"
    )
    parser.add_argument(
        "--city",
        type=str,
        help="Miasto, np. Berlin",
        default=None,
    )
...
if __name__ == "__main__":
    args = parse_args()
    breweries_list: List[Brewery] = fetch_breweries(limit=20, city=args.city)

    if args.city:
        print(f"Browary w mieście: {args.city}")
    else:
        print("Pierwsze 20 browarów:")
